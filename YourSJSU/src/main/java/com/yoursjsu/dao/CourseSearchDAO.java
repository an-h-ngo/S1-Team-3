...
/**
 *   getAllTerms() — fills the term dropdown on other pages
 *   searchSections() — searches for class sections by title keyword + dept code
 */
public class CourseSearchDAO {
    ...
    /**
     * Searches for course sections by keyword AND/OR department code.
     *
     * The keyword box matches against EITHER the course title OR the course
     * number.
     *
     * Send "%" as the value if it is empty, which mtches anything
     * That way one fixed SQL string
     * handles every combination of filled-in and blank inputs.
     *
     * Joins section, course, department, and term tables
     */
    public List<SectionResult> searchSections(String keyword, String departmentCode) {
        ...
        try (Connection conn = DatabaseConnection.getConnection();
            ...
            // Plug user inputs into SQL ? marks
            // Once for course title search and another for course number search
